...
- Output formatting: The command prints either a JSON representation or a human-readable table with columns for type, size, modification time, and name. Empty directories are handled explicitly.
...
The list command orchestrates a request to the Baidu Pan API and renders the response according to user preferences.
...
Cmd-->>User : Tabular output with headers
end
```
...
- Human-readable output: When disabled, the command prints a header row and iterates over items, displaying:
  - Type: “DIR” for directories, “FILE” otherwise
...
**Section sources**
- [src/commands/list.ts](file://src/commands/list.ts#L49-L74)
- [src/utils/index.ts](file://src/utils/index.ts#L7-L23)
...
## Conclusion
The list command provides a straightforward way to browse Baidu Pan directories with flexible sorting and output options. While it does not implement recursive traversal or explicit pagination flags, it integrates cleanly with shell pipelines and supports JSON output for downstream processing. For large directories, consider using JSON output and external filtering to optimize performance.
